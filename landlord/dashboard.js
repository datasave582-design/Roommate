...
function propertyForm(){
  ...
  $("saveProperty").onclick=async()=>{
    ...
    try{
      ...
      if(code==="permission-denied") msg="Firebase permission denied. Deploy the included Firestore rules, then sign out/in.";
      else if(code==="unauthenticated" || code==="auth/invalid-user") msg="Login session expired. Please login again.";
      else if(code==="failed-precondition") msg="Firestore is not ready. Check that Cloud Firestore is enabled in roommate-b1018.";
      else if(code==="unavailable") msg="Firebase is temporarily unavailable. Check internet and retry.";
...
async function copyLandlordCode(){
  const code=$("landlordCode")?.textContent?.trim();
  if(!code || code === "—" || code === "Loading…") return toast("Makan Malik code अभी तैयार नहीं है.");
  try{
    await navigator.clipboard.writeText(code);
    toast("✅ Makan Malik code copied");
  }catch(e){
    toast("Code: " + code);
  }
}
  ...
  onReady:async(user,profile)=>{
    ...
    try{
      ...
      toast(code === "permission-denied"
        ? "Makan Malik code blocked: deploy Firestore rules."
        : `Makan Malik code failed: ${code||"Firebase error"}`);
    }

    try{ await loadBuildings(); }
    catch(e){
      console.error("Building list load failed:",e);
      $("buildingList").innerHTML=`<div class="empty-state"><div class="emoji">⚠️</div><h3>Buildings could not be loaded</h3><p>Firebase code: <b>${esc(e?.code||"unknown")}</b></p><p style="font-size:.82rem">${esc(e?.message||"Check Firestore rules and database setup.")}</p></div>`;
      toast(`Buildings load failed: ${e?.code||"Firebase error"}`);
    }
